...
void SubOption_2_3()
{
	...
	//cout << "如果不输入分值，将以默认分值: 加[" << sys.getDefaultPlus() << "]减[" << sys.getDefaultMinus() << "]计算" << endl;
	cout << "请输入选项，及分值\n > ";
}
...
void ShowSettings()
{
	...
	cout << "自动登出时间，不小于课堂时长: " << sys.getDefaultAutoLogoutTime() << endl;
...
void ImportStuInfo()
{
	...
	int score_temp;
	/*
	cout << "请注意：数据格式应为格式 学号\t姓名\t邮箱\t分数 ! 否则会报错，之后修改";
	char *filename;
	cout << "请输入要读取的文件名：\n > ";
	cin >> filename;
	*/
...
void Sort(Grade s[], int l, int r)
{
	if (l < r)
	{
		...
		while (i < j)
		{
			while (i < j && s[j].getScore() >= x.getScore()) // 从右向左找第一个小于x的数
				j--;
			if (i < j)
				s[i++] = s[j];
			while (i < j && s[i].getScore() < x.getScore()) // 从左向右找第一个大于等于x的数
				i++;
			if (i < j)
				s[j--] = s[i];
		}
...
void SystemSettings()
{
	...
	else if (option == 2)
	{
		...
		cout << "请按以上顺序依次输入设置项: \n > " << endl;
		cin >> is_auto_calculate_temp >>
			default_minus_temp >>
			default_plus_temp >>
			default_class_time_temp >>
			default_auto_logout_time_temp >>
			default_class_temp;
